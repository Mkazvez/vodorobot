...
const Documenthouse = db.documenthouse;
const Op = db.Sequelize.Op;

function movedFileInDictory(id, fileName) {
  fs.access("state/houses/"+id+"/",async function(error) {
    if (error) {
      console.log("Directory does not exist.")
      fs.mkdir("state/houses/"+id+"/", err => {
    ...
    setTimeout(() => {
      fs.rename("state/houses/"+fileName, "state/houses/"+id+"/"+fileName, err => {
...
exports.create = (req, res) => {
    ...
    Documenthouse.create(documetnhouse)
      .then(data => {
        // const id = data.id
      ...
      .catch(err => {
        res.status(500).send({
          message:
            err.message || "Ошибка при создании записи documetnhouse."
        });
      });
...
exports.findOne = (req, res) => {
    ...
    Documenthouse.findByPk(id)
      .then(data => {
        res.send(data);
      })
      .catch(err => {
        res.status(500).send({
          message: "Error retrieving Documetnhouse with id=" + id
        });
      });
};
...
exports.downloadOne = (req, res) => {
  const id = req.params.id;
    ...
    .then(data => {
      ...
      // res.send(pathFile)
      fs.access("state/houses/"+id_house+"/", function(error) {
        if (error) {
          console.log("Directory does not exist.")
          res.status(500).send("Нет директории")
        } else {
            const file = "state/houses/"+id_house+"/"+pathFile;
            console.log(file)
...
exports.show= (req, res) => {
  //Добавьте обработчик запросов для / showURL, который напрямую жестко кодирует содержимое файла  в браузере.
    ...
    .catch(err => {
      res.status(500).send({
        message: "Error retrieving Documenthouses with id=" + id
      });
    });
...
//    console.log('body',req.body)
    await Documenthouse.update(req.body,{
      where: { id: id }
    })
      .then(async num => {
        if (num == 1) {
        ...
        //   movedFileInDictory(id, pathFile);
          ...
          res.send({
            message: `Cannot update Documenthouses with id=${id}. Maybe News was not found or req.body is empty!`
          });
        }
      })
      .catch(err => {
        res.status(500).send({
          message: "Error updating Documenthouses with id=" + id
        });
      });
};
...
exports.delete = async (req, res) => {
    ...
    await Documenthouse.findByPk(id)
    .then(data => {
    ...
    Documenthouse.destroy({
        where: { id: id }
    })
        .then(num => {
        if (num == 1) {
            res.send({
            message: "Documenthouses was deleted successfully!"
            });
            fs.unlink("state/houses/"+id_house+"/", (err) => {
              if (err) throw err;
...
exports.deleteAll = (req, res) => {
    Documenthouse.destroy({
        ...
        .then(nums => {
          res.send({ message: `${nums} Documenthouse were deleted successfully!` });
        })
        .catch(err => {
          res.status(500).send({
            message:
  ...
  //   console.log(data)
    ...
    fs.access("state/houses/"+id_house+"/",async function(error) {
      ...
      //console.log(file2.FileName)
        ...
        if (chunk.Index===0) {
          const e = fs.writeFile("state/houses/"+id_house+'/'+pathFile, req.files.file.data, function(err, result) {
            if(err) console.log('error', err);
          })}else
          {
            const e = fs.appendFile("state/houses/"+id_house+'/'+pathFile, req.files.file.data, function(err, result) {
              if(err) console.log('error', err);
            })
          }}, 1000)
